...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId = 'prop-001', debugTenant, showRaw }) => {
    ...
    const revenueStr = typeof data.total_revenue === 'string' ? data.total_revenue : String(data.total_revenue);
    const [wholePart, fractionPart = '00'] = revenueStr.split('.');
    
    const formattedWhole = Number(wholePart).toLocaleString();
    const formattedFraction = fractionPart.padEnd(2, '0').slice(0, 2);
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300">
            ...
            <div className="p-6">
                    ...
                    <div>
                        ...
                        <div className="flex items-baseline gap-2 mt-1">
                            <span className="text-3xl font-bold text-gray-900 tracking-tight">
                                {data.currency} {formattedWhole}<span className="text-xl text-gray-500">.{formattedFraction}</span>
                            </span>
                ...
                <div className="mt-4 h-6">
                    {/* Precision mismatch warning is no longer applicable since we consume string from Decimal */}
                </div>
            </div>
        </div>
    );
